...
public class MovePair {
	...
	*/
	...
	public static final int DIR_ABS_N = 4;
	/* indexes for alignments_by_dir */
	public static int	DIR_IDX_HORIZONTAL	= 0,
						DIR_IDX_DIAGRIGHT 	= 1,
						DIR_IDX_VERTICAL	= 2,
						DIR_IDX_DIAGLEFT	= 3;
	...
	//#region BOUNDS
		...
		public MovePair getDirection(MovePair target) {
			if(target.i < i) {
				if(target.j < j) 		return DIRECTIONS[5]; 
				else if(target.j == j)	return DIRECTIONS[6];
				else					return DIRECTIONS[7];
			} else if(target.i == i) {
				if(target.j < j)		return DIRECTIONS[4];
				else					return DIRECTIONS[0];
			} else {
				if(target.j < j)		return DIRECTIONS[3];
				else if(target.j == j)	return DIRECTIONS[2];
